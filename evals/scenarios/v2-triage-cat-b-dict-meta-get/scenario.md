# v2 Migration Triage — Category B: Dict meta_get Error

## Background

A user is migrating their dbt project to dbt v2. A model retrieves the `meta` config as a plain dictionary (`config.get('meta', {})`), then incorrectly calls `.meta_get()` on that plain dict. Only `config` objects have `meta_get()` — plain dicts must use `.get()`.
