# v2 Migration Triage — Category B: Unused Schema Entry

## Background

A user is migrating their dbt project to dbt v2. Their `schema.yml` defines two models (`stg_users` and `stg_orders`), but only `stg_orders` has a corresponding SQL file. v2 flags the orphaned `stg_users` entry as dbt1005.
