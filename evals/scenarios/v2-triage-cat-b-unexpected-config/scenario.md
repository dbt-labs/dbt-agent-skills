# v2 Migration Triage — Category B: Unexpected Config Keys

## Background

A user is migrating their dbt project to dbt v2. A model uses custom config keys (`my_custom_key`, `another_unexpected_key`) at the top level of the `config()` block. v2 only accepts recognized dbt config keys at the top level — custom keys must be moved to the `meta:` section.
