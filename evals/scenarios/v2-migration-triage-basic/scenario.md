# v2 Migration Triage — Basic Classification

## Background

A user is migrating their dbt project from dbt-core to dbt v2. They ran `dbt compile` and received errors because their model uses `config.require('owner')` to access a custom key that lives under `meta`. In v2, `config.require()` only works for built-in config keys. Custom keys stored in `meta` must be accessed via `config.meta_require('key_name')`.

The `dbt_compile_output.txt` file contains the real v2 compiler output showing the dbt1501 error and the dbt1000 warning that hints at the fix.
